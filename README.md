...
| `mcpServers.<name>.type` | 否 | MCP 服务传输类型，支持 `stdio` 和 `http`，默认 `stdio`。 |
| `mcpServers.<name>.command` | stdio 必填 | MCP 服务启动命令，仅适用于 `stdio` 类型。 |
| `mcpServers.<name>.url` | http 必填 | MCP HTTP 服务端点，必须为绝对 `http` 或 `https` URL，仅适用于 `http` 类型。 |
| `mcpServers.<name>.headers` | 否 | MCP HTTP 服务自定义请求头，仅适用于 `http` 类型，键值会随每次请求发送。 |
