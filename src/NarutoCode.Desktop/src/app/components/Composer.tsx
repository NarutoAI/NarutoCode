...
export function Composer({
  disabled, draft, attachments, isRunning, queuedMessageCount, approvalPending, onDraftChange, onSend, onAddImages, onPasteImage, onCancel, onRemoveAttachment, onResolveApproval,
}: ComposerProps) {
  return (
    <section className="composer-wrap">
      ...
      <div className={`composer ${disabled ? 'disabled' : ''}`}>
        <textarea aria-label="输入消息" value={draft} disabled={disabled} placeholder={disabled ? '请先选择一个会话' : '询问、描述任务，或粘贴代码、图片…'} onChange={event => onDraftChange(event.target.value)} onPaste={event => {
        ...
        <div className="composer-footer">
          ...
          <button className="send-button" type="button" aria-label="发送消息" disabled={disabled || (!draft.trim() && attachments.length === 0)} onClick={onSend}><ArrowUp size={18} weight="bold" /></button>
        </div>
      </div>
    </section>
  )
